...
class ResNet18(nn.Module):
    def __init__(self, input_channel, output_dim, learning_rate, reg_lambda, device, use_batch_norm=False, use_mixup=False, mixup_alpha=0.2):
        ...
        self.CONV3 = nn.Conv2d(in_channels=64, out_channels=128,kernel_size=(3,3),stride=2,padding=1)
        self.CONV4 = nn.Conv2d(in_channels=128, out_channels=256,kernel_size=(3,3),stride=2,padding=1)
        self.CONV5 = nn.Conv2d(in_channels=256, out_channels=512,kernel_size=(3,3),stride=2,padding=1)

        self.avg_pool = nn.AdaptiveAvgPool2d((1,1))
        self.fc = nn.Linear(512 * 1, 100)
    ...
    def restore(self):
        with open(os.path.join('./best_model/ResNet18.pt'), 'rb') as f:
